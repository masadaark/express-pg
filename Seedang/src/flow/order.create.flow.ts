...
export class CraeteOrderFlow{
    ...
    async CreateOrder(req: CreateOrderModel, userId: number) : Promise<{ orderId: number; }> {
        ...
        const resultTransactionSave = (await getDB().insert([transactionSave]).into("seedang.transaction").returning('*'))[0];
        const rateIncome = (await getFromQuery("select p.* from seedang.income_rate p where p.package_name =" + req.package.packageName))[0]
        let coverage = (await getFromQuery("select p.* from seedang.coverage p where p.coverage_detail =" +{coverage : req.package.coverages}))[0];
        if(!coverage){
        ...
        console.log(rateIncome);
        console.log(resultTransactionSave);

        const orderSave : OrderTable = OrderCreateLogic.mapOrder(req, rateIncome.id, coverage.id, resultTransactionSave.id, userId);
        const resulatOrder = await getDB().insert(orderSave).into("seedang.coverage").returning('*')
