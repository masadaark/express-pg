...
export class OrderFlow {
    ...
    static async CreateOrder(req: CreateOrderModel, userId: number): Promise<{ orderId: number; }> {
        if (dayjs(req.startDate).startOf('day').isBefore(dayjs().startOf('day'))) {
            throw createError({ status: 400, message: "ไม่สามารถซื้อประกันเริ่มคุ้มครองก่อนปัจจุบันได้" })
        }
    ...
    static async CreateInsurance(req: CreateInsurance, userId: number): Promise<{ orderId: number, paymentAmount: number; }> {
        const transactionSave = OrderCreateLogic.maptransaction(userId, 2);
        const insuranceValidate = await InsuranceFlow.validInsurance(req.orderId)
        if (insuranceValidate){
            throw createError({
                status: 400, message: "มีรายการนี้แล้วในระบบ"
            })
        }
        const resultTransactionSave = (await getDB().insert([transactionSave]).into("seedang.transaction").returning('*'))[0];
    ...
    static async GetOrderState(userId: number) {
        ...
        return await getDB().from('seedang.order as o')
            .select('o.id as orderId', 'o.origin_country as originCountry', 'o.destination_country as destinationCountry', 'o.start_date as startDate', 'o.end_date as endDate', 'o.package_name as packageName', 'logorder.ostatus as statusId', 'logorder.oname as statusName')
            .join(
                getDB()
